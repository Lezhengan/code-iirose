(function () {
    ...
    function getInnerCode() {
        ...
        function touchInput(evt, viaBtn) {
            try {
                var ae = document.activeElement;
                var inp = ae && ae.tagName === 'TEXTAREA' ? ae : document.getElementById('moveinput');
                if (!inp || !inp.value.trim()) return;
                var before = inp.value;
                // skip command inputs (@ / @@ / <> / ~ / #)
                if (/^[@<>~#]/.test(before)) return;
                var after = processText(before);
                if (after !== before || viaBtn) {
                    if (evt && evt.preventDefault) { evt.preventDefault(); evt.stopImmediatePropagation(); }
                    var send = after !== before ? after : before;
        ...
        function hookSendBtn() {
            try {
                var btns = document.querySelectorAll('.moveinputSendBtn, .homeHolderMsgContentInputSend');
        ...
        function hookEnter() {
            try {
                var sels = ['moveinput', 'homeHolderMsgContentInputBox'];
                for (var x = 0; x < sels.length; x++) {
                    var inp = document.getElementById(sels[x]);
        ...
        function buildUI(){
            ...
            pl.innerHTML=[
                ...
                '</div>',
                '</div>',
                '<div class=st id=keigo-st>in:'+(_hooked?'\u2713':'\u2717')+'</div>'
            ].join('');
